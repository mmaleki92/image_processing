...
def process_segmentation(image, bg_image=None, bg_color=None, threshold=0.1):
    ...
    # Expand mask dimensions to 3 channels for foreground extraction
    binary_mask_3d = np.stack((binary_mask,) * 3, axis=-1)
    
    # Extract the foreground (person) from the image
    foreground = np.zeros_like(image)
    foreground = np.where(binary_mask_3d, image, foreground)
    ...
    if bg_image is not None:
        ...
        # Handle if bg_image has an alpha channel (PNG transparency)
        if bg_image.shape[2] == 4:
            # Convert 4-channel BGRA to 3-channel BGR
            alpha_channel = bg_image[:,:,3] / 255.0
            alpha_channel = np.repeat(alpha_channel[:,:,np.newaxis], 3, axis=2)
            
            rgb_channels = bg_image[:,:,0:3]
            
            # Create a white background
            white_bg = np.ones_like(rgb_channels) * 255
            
            # Blend the image with a white background based on alpha
            bg_image = rgb_channels * alpha_channel + white_bg * (1 - alpha_channel)
            bg_image = bg_image.astype(np.uint8)
            
        # Combine foreground with the background image
        output_image = bg_image.copy()
        # Place foreground (person) on top of background
        foreground_mask = cv2.cvtColor(foreground, cv2.COLOR_BGR2GRAY) > 0
        foreground_mask_3d = np.stack((foreground_mask,) * 3, axis=-1)
        output_image = np.where(foreground_mask_3d, foreground, output_image)
        ...
        output_image = np.where(binary_mask_3d, foreground, bg_image)
...
def load_background_image(filename):
    """Load background image with support for PNG transparency"""
    if not os.path.exists(filename):
        print(f"Warning: Background image {filename} not found")
        return None
    ...
    print(f"Successfully loaded background image: {filename}")
    return image
...
def main():
    ...
    print("Press 'l' to load a custom PNG background")
    ...
    bg_files = ['harris_corner_comparison.png']
    for bg_file in bg_files:
        custom_bg = load_background_image(bg_file)
        if custom_bg is not None:
            bg_options.append(custom_bg)
            print(f"Default background image '{bg_file}' loaded")
            break
    ...
    while cap.isOpened():
        ...
        cv2.putText(frame, "B: Change background | L: Load PNG | +/-: Adjust threshold | Q: Quit", 
                   (10, frame.shape[0] - 10), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
        ...
        if key == ord('q'):
            ...
            # Load a custom PNG image
            filename = input("Enter PNG image filename: ")
            custom_bg = load_background_image(filename)
            if custom_bg is not None:
                # Remove any previous custom backgrounds
                bg_options = [option for option in bg_options if not isinstance(option, np.ndarray)]
                # Add the new background
                bg_options.append(custom_bg)
                bg_option_index = len(bg_options) - 1
